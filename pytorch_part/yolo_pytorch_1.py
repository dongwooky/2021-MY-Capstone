...
def create_model(blocks):
    ...
    for i, block in enumerate(blocks[1:]):
        ...
        if (block["type"] == 'convolutional'):
            ...
            seq.add_module("conv_{}".format(i), conv)
            if ("batch_normalize" in block):
                bn = nn.BatchNorm2d(filters)
                seq.add_module("batch_norm_{}".format(i), bn)
            if (block["activation"] == "leaky"):
                activn = nn.LeakyReLU(0.1, inplace=True)
                seq.add_module("leaky_{}".format(i), activn)
            ...
            route = DummyLayer()
            seq.add_module("route_{}".format(i), route)
        elif (block["type"] == "shortcut"):
            from_ = int(block["from"])
            shortcut = DummyLayer()
            seq.add_module("shortcut_{}".format(i), shortcut)
            ...
            seq.add_module("Detection_{}".format(i), detectorLayer)
...
def prediction(x, inp_dim, anchors, num_classes, CUDA=False):
    ...
    prediction = prediction.trasnpose(1, 2).contiguous()
