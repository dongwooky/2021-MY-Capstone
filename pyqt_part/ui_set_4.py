...
class App(QWidget, ui):
    def __init__(self):
        ...
        self.thread_refrigerator.change_pixmap_signal_table.connect(self.update_image_table)
        # start the thread
    ...
    def update_image_refrigerator(self, frame):
        ...
        qt_img = self.convert_cv_qt_refrigerator(frame)
        self.label_refrigerator_frame.setPixmap(qt_img)
        
    @pyqtSlot(np.ndarray)
    def update_image_table(self, frame):
        """Updates the image_label with a new opencv image"""
        qt_img = self.convert_cv_qt_table(frame)
        self.label_table_frame.setPixmap(qt_img)
    
    def convert_cv_qt_refrigerator(self, frame):
    ...
    def convert_cv_qt_table(self, frame):
        """Convert from an opencv image to QPixmap"""
        rgb_image = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        h, w, ch = rgb_image.shape
        bytes_per_line = ch * w
