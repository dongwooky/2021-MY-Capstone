...
#    video_path = FLAGS.video
 ...
 #   print("Video from: ", video_path )
    ...
    while True:
        ...
        image, pred_classes = utils.draw_bbox(frame, pred_bbox)
        pred_classes = list(set(pred_classes))
        pred_text = open('./cloud/fridge_list.txt', 'w')
        for pred_class in pred_classes:
            pred_text.write(pred_class)
            pred_text.write('\n')
        pred_text.close()
        s3.upload_file('./cloud/fridge_list.txt', bucket_name, 'fridge_list')
        print(pred_classes)
        curr_time = time.time()
        ...
        if not FLAGS.dis_cv2_window:
            ...
            cv2.imwrite('./cloud/fridge_frame.png', result)
            s3.upload_file('./cloud/fridge_frame.png', bucket_name, 'fridge_frame')
