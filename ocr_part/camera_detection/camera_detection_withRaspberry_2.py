...
class CameraDetection:
    ...
    def find_rectangle(self, frame, contours):
        for points in contours:
            if cv2.contourArea(points) < 10000:  #크기가 1000이하인 contour들은 무시
                continue
            ...
            if cv2.contourArea(points) > self.frameWidth * self.frameHeight * 0.01: #일정 크기 이상이면 찾기 스탑
                return True         
    ...
    def camera_detect(self):
        while True:
            s3.download_file(bucket_name, 'table_raw', './cloud/table_raw.png')  #웹캠에서 프레임 가져오기
            self.frame = cv2.imread('./cloud/table_raw.png')
